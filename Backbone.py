...
class Block(nn.Module):
    ...
    def __init__(self, n_in, n_out, filter_sz=3, stride=1, block_sz=3):
        ...
        mods = []
        mods.extend([nn.BatchNorm2d(n_in), nn.Conv2d(n_in, n_out, kernel_size=filter_sz,
                                                     stride=self.stride, padding=1, bias=True), nn.ReLU(inplace=True)])
        for _ in range(block_sz-1):
            mods.extend([nn.BatchNorm2d(n_out), nn.Conv2d(n_out, n_out, kernel_size=filter_sz,
                                                          stride=self.stride, padding=1, bias=True), nn.ReLU(inplace=True)])
...
class RFM(nn.Module):
    ...
    def __init__(self, n_in):
        ...
        self.seq = nn.Sequential(
            nn.Linear(n_in, n_in), nn.ReLU(inplace=True),
            nn.BatchNorm1d(n_in),
            nn.Linear(n_in, n_in), nn.ReLU(inplace=True)
        )
...
class DAL_model(nn.Module):
    ...
    def __init__(self, head, n_cls, embedding_size=512, init_method={'method': nn.init.kaiming_normal_, 'paras': {}}):
        ...
        if head.lower() in 'cosface':
            self.margin_fc = CosMargin(n_cls, embedding_size, s=32., m=0.05)  # 32 0.1 worked
        ...
        self.age_classifier = nn.Sequential(
            ...
            nn.Linear(embedding_size, 1) # len(age_cutoffs)+1)
        )
        self.id_cr = nn.CrossEntropyLoss()
        self.age_cr = nn.L1Loss() # nn.CrossEntropyLoss()
    ...
    def forward(self, xs, ys=None, agegrps=None, emb=False):
        ...
        # logits = self.margin_fc(embs_id)
        # print(f'logits:\n{logits.size()}\n{logits}')
        id_acc = accuracy(torch.argmax(logits, dim=1), ys)
        # print(f'id_acc:\n{id_acc.size()}\n{id_acc}')
        idLoss = self.id_cr(logits, ys)
        # print(f'idLoss:\n{idLoss.size()}\n{idLoss}')
        # age classifier
        age_logits = self.age_classifier(embs_age)[:, 0]
        # print(f'age_logits:\n{age_logits.size()}\n{age_logits}')
        # age_acc = accuracy(torch.argmax(age_logits, dim=1), agegrps)
        age_acc = torch.tensor(1.)
        # print(f'age_acc:\n{age_acc.size()}\n{age_acc}')
